...
public class NioReactorSocketServer {
     ...
     * 但是这个新的socket channle并没有在selector"选择器/迭代器"中注册
     * 所以程序还没法通过selector通知这个socket channel的事件
     * 于是我们拿到新的socket channel后，要做的第一个事情就是到selector"选择器/迭代器"中注册这个
     *
     * @param socketChannel
     * @param selector
     */
    ...
    private static void readSocketChannel(SelectionKey readyKey) throws IOException {
        ...
        if (message.indexOf("over")!=-1) {
            ...
            //回发数据，并关闭channel
            ByteBuffer sendBuffer = ByteBuffer.wrap(URLEncoder.encode("你好，客户端，这里是服务器的返回数据","UTF-8").getBytes());
